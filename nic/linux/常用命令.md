...
excli network firewall set --enabled false
 esxcli software component apply -v /tmp/VMW-esx-7.0.0-VMware-nvmxnet3-2.0.0.30-1vmw.700.1.0.15511075.zip
...
# ib 模式网卡不显示
/opt/mellanox/bin/mlxconfig -d mt4119_pciconf0 set ADVANCED_PCI_SETTINGS=1 
/opt/mellanox/bin/mlxconfig -d mt4119_pciconf0 set FORCE_ETH_PCI_SUBCLASS=1
```
...
```shell
esxcli qlfc qcc adapter list # fc id 查询
esxcli qlfc qcc adapter info get -i Adapter_QLE2692_RFD1720T20529 # fw version
esxcli storage san fc list # dr version
esxcli qlfc qcc adapter update -i Adapter_QLE2692_RFD1720T20529  FC-HBA-QLE2692-16Gb-2P/fw/bk010606.bin -m MBI # fw 升级

# internal 
 esxcli qlfc qcc port loopback run -i FC_21-00-00-24-FF-17-A8-2A -p 111 -z 4096 -l Internal -n 3 -e Stop -t 999 # 可选参数
